...
# when set, action will be editing a tag, and this will be tag name to edit
my $x_edit_tag_value;
# when set, action is to edit version and this is what will be the new Version: value
my $x_edit_version_value;
# When set, action is to search %global(define) value
my $x_get_global_name;
# Used for --value parameter when editing a tag
my $x_value;
...
Operating mode:
 ...
 -e, --edit-tag x --value y      updates Tag: with specific value (the tag
     ...
     --ev, --edit-version x      updates Version: tag
...
if ($x_edit_version_value) {
    $command = 'editversion';
}

if ($x_get_global_name) {
    $command = 'getglobal';
}
